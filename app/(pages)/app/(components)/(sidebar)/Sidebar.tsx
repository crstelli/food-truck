...
function Sidebar() {
  ...
  return (
    <aside className="grid grid-cols-[auto_1fr] absolute top-0 left-0 h-screen w-full max-w-160">
      <nav
        className={`relative bg-secondary py-15 z-401 transition-[width] duration-300 ease-in-out items-start grid grid-rows-[1fr_3fr] ${
          menu ? "w-15 md:w-55" : "w-15"
        }`}
      >
        <Link
          href="/"
      ...
      <div
        className={`py-15 overflow-y-auto flex flex-col relative transition-[width] duration-300 ease-in-out bg-white dark:bg-gray-900 ${
          menu ? "w-full sm:w-110 px-4 z-401" : "w-0 px-0"
        }`}
      >
        <button
          onClick={closeMenu}
